...
- 知识库空间（阶段1）：`/knowledge-spaces` 空间 CRUD（用户级隔离），`knowledge.space_id` / `agent.kb_*`
  字段，向量集合键 `agent_{id}` / `space_{id}` 双制式，存量迁移脚本。前端「知识库中心」页。
  文档按空间管理、多空间联合检索、引用来源见 `docs/knowledge-space-plan.md` 阶段 2/3。

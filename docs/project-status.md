...
- 知识库空间（阶段1-3）：`/knowledge-spaces` 空间 CRUD + 空间内文档管理（用户级隔离），
  ...
  未绑定空间的旧 Agent 回退原私有库路径。前端「知识库中心」/「空间详情」页 + Agent 编辑「知识库」区块 +
  聊天页「参考来源」。阶段 4+（调试台 / 健康分 / 企业权限）见 `docs/knowledge-space-plan.md`。

...
- 备份恢复：已在部署文档说明 MySQL 导出与应用文件目录打包方式。
- 数据库迁移：已加入 Alembic 迁移骨架、基线版本和迁移文档，当前处于兼容过渡期。
- 部署基础：进程管理器常驻 uvicorn + worker，Nginx 托管前端，生产环境模板与部署文档。
...
- 压力测试还未在真实服务器上形成基准报告。
- `/metrics` 已可接入 Prometheus，但还缺真实生产压测基线和告警规则。
...
1. 测试数据库下的接口级回归测试。
2. 真实服务器压力测试基准报告。
3. 拆分 ORM 模型定义与数据库启动初始化，完成 Alembic 全量接管。
4. 备份恢复演练和告警通知接入。
